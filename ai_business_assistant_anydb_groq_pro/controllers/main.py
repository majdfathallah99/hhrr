...
class AIAssistantController(http.Controller):
    ...
    # ----------- Routes -----------
\2("/ai_assistant_ping", type="http", auth="user")
    def ai_ping(self, **kw):
        return request.make_response("OK", headers=[("Content-Type", "text/plain; charset=utf-8")])
\2("/ai_assistant/query", type="json", auth="user", csrf=False)
    def ai_query(self, **kwargs):
        try:
            ...
            return {"error": str(e)}
\2("/ai_assistant/query_http", type="http", auth="user", csrf=False, methods=["POST", "GET"])
...
\2("/ai_assistant_diag", type="http", auth="user")
    def ai_diag(self, **kw):
        cfg = self._provider_config()
        redacted_cfg = {
...
\2("/ai_assistant_plain_js", type="http", auth="user")
...
\2("/ai_assistant_plain", type="http", auth="user")
