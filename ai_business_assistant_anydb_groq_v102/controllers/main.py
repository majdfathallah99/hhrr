...
def _core_ai_logic(self, message):
    ...
    sys_prompt = "You are an Odoo assistant. Use tools to answer and create/update records. Keep answers short and plain."
    payload = {
        "model": model,
        "messages": [
...
def ai_query_http(self, **kw):
    try:
        # Read message from JSON body or querystring
        ...
        if not message:
            message = request.httprequest.args.get("message")
        if not message:
            return request.make_response('{"error":"Missing parameter: \'message\'"}', headers=[("Content-Type","application/json")], status=200)
        if message.strip().lower() in ("hi","hello","hey"):
            return request.make_response('{"text":"Hi! Ask me about your Odoo data or tell me to create something.","message":"Hi! Ask me about your Odoo data or tell me to create something.","tool_calls":[],"tool_results":[]}', headers=[("Content-Type","application/json")], status=200)
        ...
        return request.make_response('{"error":"%s"}' % str(e).replace('"','\"'), headers=[("Content-Type","application/json")], status=200)
...
def ai_query_json(self, message=None, **kw):
    try:
        ...
        if not message:
            return {"error": "Missing parameter: 'message'"}
        if message.strip().lower() in ("hi","hello","hey"):
            return {"text":"Hi! Ask me about your Odoo data or tell me to create something.","message":"Hi! Ask me about your Odoo data or tell me to create something.","tool_calls":[],"tool_results":[]}
