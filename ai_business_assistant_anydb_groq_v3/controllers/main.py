...
class AIAssistantController(http.Controller):
    ...
    @http.route("/ai_assistant/query_http", type="http", auth="user", csrf=False, methods=["POST"])
    def ai_query_http(self, **kwargs):
        # Accept {"message":"..."} or {"params":{"message":"..."}}
        try:
            ...
            if "message" not in params:
                params = {"message": (params.get("params") or {}).get("message")}
            # Call the JSON route handler directly and wrap result
...
async function sendMsg(message){
  ...
  try{
    var res=await fetch('/ai_assistant/query_http',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin',body:JSON.stringify({message:message})});
    var raw = await res.text();
    var data = null;
    try { data = JSON.parse(raw); } catch(e){ data = { error: 'Invalid JSON from server', debug: raw }; }
    var payload = (data && data.result) ? data.result : data; // unwrap JSON-RPC if present

    if(!res.ok){
      var msg = (payload && (payload.error||payload.message)) ? (payload.error||payload.message) : raw;
      appendLog(String(msg),'Assistant'); return;
    }
    if(payload && payload.text){
      appendLog(payload.text,'Assistant'); speak(payload.text); return;
    }
    if(payload && payload.error){
      appendLog('Error: '+(typeof payload.error==='string'?payload.error:JSON.stringify(payload.error)),'Assistant'); return;
    }
    // Fallback friendly message instead of dumping raw JSON
    appendLog('I received a response but it didn\'t include a message. Try rephrasing your question.', 'Assistant');
  }catch(err){
