...
def _save_history(self, hist):
    ...
    def _core_ai_logic(self, message: str):
        ...
        hist = self._get_history(); hist.append({"role":"user","content": message}); self._save_history(hist);
        ...
        sys_prompt = ("You are an Odoo business assistant. Maintain conversation context across messages. Only call write/create/delete tools when the user explicitly requests it and after you have enough details (customer/vendor for PO vs customer for SO, lines). Ask concise follow-up questions to gather missing details. Never invent IDs, partners, or products. For domains and lines, output proper JSON arrays, not strings. Keep answers short and plain.")

        payload = {
            "model": model,
            "messages": [{"role":"system","content":sys_prompt}] + self._get_history() + [{"role":"user","content":message}],
            "tools": tool_schemas,
            "tool_choice": "auto",
            "temperature": 0.2,
        }
        ...
        if tool_calls:
            ...
            hist2 = self._get_history(); hist2.append(msg); hist2.extend(tool_msgs); self._save_history(hist2); payload2 = {"model": model, "messages": [{"role":"system","content":sys_prompt}] + hist2, "temperature": 0.2}
            try:
            ...
            hist = self._get_history(); hist.append({"role":"assistant","content": text}); self._save_history(hist); return {"text": text, "message": text, "tool_calls": tool_calls, "tool_results": tool_results}
