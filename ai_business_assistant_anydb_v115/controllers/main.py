...
class AIAssistantController(http.Controller):
    """Stable endpoints + minimal HTML chat (stateful, chat-any fallback)."""
...
def _ensure_product(self, name: str):
    ...
    if p:
        return p
    # create simple stockable product with Unit UoM if possible
    try:
...
def _parse_qty_and_product(self, message: str):
    # naive: first integer = qty; product words after it
    import re
...
def _fastpath(self, message: str):
    ...
    if "create" in msg and ("sale order" in msg or "sales order" in msg or "so " in msg or msg.endswith(" so")):
        # customer name after 'for ' ... or previously remembered
        customer = None
        # try 'for X'
        import re
        ...
        if not customer:
            # fallback: last word before 'sell' 'buy' 'contains' or end
            pass
        if not customer:
        ...
        # if we have only customer or only product, remember and ask minimal follow-up
    ...
    def _core_ai_logic(self, message: str):
        ...
        # Tools may not be available on first load: fall back to plain chat
        tool_schemas = []
        try:
        ...
        if not choices:
            txt = "(no content)"
            hist = self._get_history(); hist.append({ "role": "assistant", "content": txt }); self._save_history(hist)
        ...
        if tool_calls and tool_schemas:
            ...
            try:
                ...
                hist = self._get_history(); hist.append({ "role": "assistant", "content": text }); self._save_history(hist)
            ...
            # add model message + tool outputs to history
