{
    "application": true,
    "author": "You + ChatGPT",
    "category": "Productivity",
    "data": ["security/ir.model.access.csv", "data/set_groq_defaults.xml", "data/set_groq_key.xml", "data/ai_app_menu.xml", "data/voice_menu.xml", "views/voice_menu.xml", "views/res_config_settings_ai_voice.xml"],
    "depends": ["account", "base", "base_setup", "purchase", "sale", "sale_management", "stock", "web", "website"],
    "installable": true,
    "license": "LGPL-3",
    "name": "AI Business Assistant v118 (Agentic Multi‑Turn + Name Guard)",
    "summary": "Ask anything about your Odoo data; safe generic tools + Groq defaults (OpenAI-compatible).",
    "version": "17.0.118.0",
    "website": "https://example.com"
}
