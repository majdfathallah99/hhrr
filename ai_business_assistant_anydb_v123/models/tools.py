...
def _find_partner(self, partner_id=None, partner_name=None, is_vendor=False, autocreate=False):
    ...
    if partner_name:
        ...
        if p:
            return p
        if autocreate and is_vendor:
            vals = {"name": partner_name, "supplier_rank": 1}
            try:
                return Partner.create(vals)
            except Exception:
                pass
        if autocreate and not is_vendor:
...
def _find_product(self, product_id=None, default_code=None, name=None, autocreate=False):
    ...
    if name:
        ...
        if p:
            return p
        if autocreate:
            # auto-create product if permitted
            try:
                Uom = self.env["uom.uom"].sudo()
                unit = Uom.search([("uom_type", "=", "reference"), ("category_id.name", "ilike", "Unit")], limit=1)
                vals = {
                    "name": name,
                    "default_code": default_code or False,
                    "detailed_type": "product",
                    "uom_id": unit.id if unit else False,
                    "uom_po_id": unit.id if unit else False,
                }
    ...
    def _create_sale_order(self, params):
        ...
        partner = self._find_partner(params.get("customer_id"), params.get("customer_name"), is_vendor=False, autocreate=True)
        if not partner:
        ...
        for l in lines:
            ...
            prod = self._find_product(l.get("product_id"), l.get("default_code"), l.get("name"), autocreate=True)
    ...
    def _create_purchase_order(self, params):
        params = dict(params or {})
        lines = self._coerce_lines(params.get("lines"))
        partner = self._find_partner(params.get("vendor_id"), params.get("vendor_name"), is_vendor=True, autocreate=True)
        if not partner:
