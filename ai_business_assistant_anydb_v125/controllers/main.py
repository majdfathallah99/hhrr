...
<div class="wrap">
  ...
  <div class="controls">
    ...
    <a id="voiceBtn" href="/ai_assistant/voice" style="margin-left:8px; padding:10px 16px; border-radius:8px; background:#6b7280; color:#fff; text-decoration:none;">🎙 Voice mode</a>
  </div>
  <p class="meta">Text chat calls <code>/ai_assistant/query_http</code>. Voice uses <code>/ai_assistant/voice</code>.</p>
</div>
<script>
...
(function(){
  ...
  const holdBtnHtml = '<button id="voiceHold" style="margin-left:8px;">Hold to talk</button>';
  // place the hold button if Send exists
  try {
    const controls = document.querySelector('.controls') || document.body;
    if (controls && !document.getElementById('voiceHold')) {
      const sendBtn = document.getElementById('sendBtn') || document.getElementById('send');
      if (sendBtn) {
        sendBtn.insertAdjacentHTML('afterend', holdBtnHtml);
      } else {
        const ta = document.getElementById('message');
        if (ta) ta.insertAdjacentHTML('afterend', holdBtnHtml);
      }
    }
  } catch(e) {}
  ...
  const btn = document.getElementById('voiceHold');
  const msg = document.getElementById('message');
  const sendBtn = document.getElementById('sendBtn') || document.getElementById('send');

  if(btn && msg && sendBtn){
    let listening = false;
    const start = ()=>{ if(!listening){ listening = true; btn.textContent = 'Listening…'; recog.start(); } };
    const stop = ()=>{ if(listening){ listening = false; btn.textContent = 'Hold to talk'; recog.stop(); } };
    ...
    recog.onend = ()=>{ listening=false; btn.textContent='Hold to talk'; };
    recog.onerror = ()=>{ listening=false; btn.textContent='Hold to talk'; };
  }
})();
</script>
