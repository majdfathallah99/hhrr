...
class AIAssistantController(http.Controller):
    ...
    def _naive_intent(self, message):
        ...
        if any(k in msg for k in ["top", "bestseller"]) and "product" in msg:
            from datetime import date, timedelta
            end = date.today()
            start = (end - timedelta(days=30)).isoformat()
            return ("top_selling_products", {"start_date": start, "end_date": end.isoformat(), "limit": 10})
        if any(k in msg for k in ["revenue", "sales"]):
            from datetime import date
            start = date(date.today().year, 1, 1).isoformat()
            end = date.today().isoformat()
            return ("get_total_revenue", {"start_date": start, "end_date": end, "journal_types": ["sale"]})
        if "expense" in msg:
            from datetime import date
            start = date(date.today().year, 1, 1).isoformat()
            end = date.today().isoformat()
            return ("get_total_expense", {"start_date": start, "end_date": end})
        return (None, None)
class AIAssistantController(http.Controller):
    ...
    def ai_query(self, **kwargs):
        ...
        try:
            ...
            if not final_text:
                # fallback to naive intent if possible
                name, args = self._naive_intent(message)
                if name:
                    result = request.env["ai.business.tools"].sudo().execute_tool(name, args)
                    final_text = f"Result for {name}: " + json.dumps(result)
                    ...
                    final_text = "I couldn't find a direct answer. Try asking about profit, revenue, expenses, product counts, or top products."
