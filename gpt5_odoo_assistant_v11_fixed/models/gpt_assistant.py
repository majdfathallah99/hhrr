...
class GptAssistantThread(models.Model):
    ...
    def _check_super(self):
        for rec in self:
            if (rec.sudo_mode or rec.allow_all_models) and not self.env.user.has_group('gpt5_odoo_assistant_v8_fixed.group_gpt_assistant_superuser'):
    ...
    def _env_for_tools(self):
        return self.env.sudo() if (self.sudo_mode and self.env.user.has_group('gpt5_odoo_assistant_v8_fixed.group_gpt_assistant_superuser')) else self.env
