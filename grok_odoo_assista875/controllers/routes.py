...
<body>
  ...
  <table border="0" cellpadding="4" cellspacing="0">
    <tr>
      <td>
        <button id="voice" onclick="try{ if(window.Grok&&Grok.onVoiceClick){Grok.onVoiceClick(); return false;} }catch(e){}" type="button">🎤 Start / ابدأ</button>
...
<noscript><div>⚠️ JavaScript معطّل في المتصفح — لن تعمل الأزرار.</div></noscript>
  <hr/>
  <details>
...
APP_JS = """(function(){
  window.Grok = window.Grok || {};

  // Diagnostics utility
  function diag(msg){
    try{
      var box=document.getElementById('log');
      if(box){ var d=document.createElement('div'); d.textContent=msg; box.appendChild(d); }
      console.log('[GROK]', msg);
    }catch(e){}
  }
  // Expose handlers for inline onclick
  Grok.onSendClick = function(){
    try{
      var i=document.getElementById('input');
      if(!i) return diag('input#input not found');
      var t=(i.value||'').trim();
      if(!t){ diag('empty message'); return; }
      add('user', t);
      sendText(t);
      i.value='';
    }catch(e){ diag('sendClick error: '+e.message); }
  };
  Grok.onVoiceClick = function(){
    try{
      if(!SR){ setupSR(); }
      if(!SR){ diag('SpeechRecognition unavailable'); alert('🎤 غير متاح في هذا المتصفح'); return; }
      if(SR.listening){ SR.stop(); diag('SR stop'); }
      else { SR.start(); diag('SR start'); }
    }catch(e){ diag('voiceClick error: '+e.message); }
  };
...
');
  ...
  function init(){
    try { window.GROK_READY = false; } catch(e) {}
    bindUI();
