...
SYSTEM_PROMPT = (
    "أنت مساعد أودو. رد دائمًا باللغة العربية فقط. لا تستخدم أي لغة أخرى.\n"
    "عند طلب المستخدم تنفيذ تغييرات أو قراءة بيانات، أخرج كتلة COMMAND بهذا الشكل:\n"
    "<<COMMAND>>\n"
    ...
    "model=<odoo.model> (فقط لـ create/write/read)\n"
    "id=<record_id> (للـ write و add_*_lines)\n"
    "values=<k=v;...> (لإنشاء/تعديل أو الأوامر المتخصصة)\n"
    "lines=<item1|item2|...> كل عنصر NAME*QTY[@PRICE] مثل: 'Blue Pen*3@12.5' أو 'PEN-001*2'\n"
    "search_name=<text> (للقراءة)\n"
    "<<END>>\n"
    "فضّل create_product للمنتجات الجديدة و create_sale_order/create_purchase_order للأوامر."
)
...
class GrokAssistantSession(models.Model):
    ...
    # === Arabic NLU helpers (rule-based pre-processing) ===
    def _normalize_ar(self, text):
        t = (text or '').strip()
        # unify forms
        t = t.replace('\u0640','')  # tatweel
        t = re.sub(r'[\u0617-\u061A\u064B-\u0652]', '', t)  # harakat
    ...
    def _contains_any(self, t, words):
        return any(w in t for w in words)

    def _pre_rule_reply(self, user_text):
        """
        Returns a tuple (handled: bool, reply_text: str)
        If handled=True, caller should return reply_text directly (skip LLM).
        """
        t = self._normalize_ar(user_text)

        # 1) "How old am I?" -> clarify we don't know user's age
        if self._contains_any(t, ['كم عمري','ما عمري','ما هو عمري','كم سني','سني كام','سني']) or (('عمري' in t) and ('كم' in t or '?' in user_text)):
            reply = _("لا أعرف عمرك لأنني لا أملك معلوماتك الشخصية. أخبرني تاريخ ميلادك (يوم/شهر/سنة) وسأحسبه لك فورًا.")
            return True, reply

        # 2) Generic filler like "ماذا؟" after confusion
        if t in ('ماذا','شنو','شنيا','هاه','شنو قصدك','شنقصدك','ما فهمتش','مش مفهوم','شن تبي'):
            reply = _("تمام. ممكن توضح طلبك بكلمة أو كلمتين؟ مثلاً: 'إنشاء منتج جديد باسم ...' أو 'اعرض طلبات البيع اليوم'.")
            return True, reply

        # 3) "I want a product" -> create product.template (not res.partner)
        if self._contains_any(t, ['اريد منتج','ابي منتج','ابي سلعه','ابي ماده','انشاء منتج','اضف منتج','اضافة منتج','عايز منتج','نبي منتج','منتج جديد']) or (t.strip() == 'منتج'):
            # Try to extract a name if user wrote: "اريد منتج كيبورد" etc.
            name = None
            m = re.search(r'(?:منتج|سلعه|ماده)\s+(.+)', t)
            if m:
                name = m.group(1).strip()
                # drop very short connectors
                if len(name) < 2 or name in ('جديد','بس'):
                    name = None
            if not name:
                name = _("منتج جديد")
            values = f"name={name}&type=product&sale_ok=True&purchase_ok=True"
            cmd = f"<<COMMAND>> action=create model=product.template values={values} <<END>>"
            reply = _("تم—سأجهّز لك منتجًا جديدًا. إن أردت اسمًا مختلفًا أو سعرًا أو فئة، أخبرني.\n") + cmd
            return True, reply
    ...
    def _call_llm(self, user_text):
        ...
        # Pre-processing rules: handle common Arabic intents before calling LLM
        handled, pre_reply = self._pre_rule_reply(user_text)
        if handled:
            return pre_reply
    ...
    # Helpers, specialized actions, parser, sanitizer, executor — (same as fallback)
# ---------- Helpers ----------
    ...
    def _sanitize_command(self, data, values):
        # Correct common model confusions from the LLM
        act = (data.get('action') or '').strip()
        mdl = (data.get('model') or '').strip()

        # If creating a product as product.product -> use product.template
        if act == 'create' and mdl == 'product.product':
            data['model'] = 'product.template'

        # If action implies product creation but model wrong (e.g., res.partner)
        if act == 'create' and mdl in ('res.partner',''):
            # Heuristic: if product-ish fields present, switch to product.template
            keys = '&'.join([values or '', (data.get('values') or '')]) if isinstance(values, str) else '&'.join(values.keys())
            try:
                ktext = keys.lower()
            except Exception:
                ktext = str(keys)
            if re.search(r'(type|list_price|standard_price|uom_id|categ_id|sale_ok|purchase_ok|default_code|barcode)', ktext):
                data['model'] = 'product.template'
