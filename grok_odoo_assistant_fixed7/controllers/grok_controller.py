...
<html>
<head><meta charset="utf-8"><title>Grok Settings</title></head>
<body>
<h1>Grok Settings</h1>
<form method="post" action="/grok/config/save">
  <div>
    <label>x.ai API Key</label><br/>
  ...
  <div>
    <label>Allowed Models (comma separated)</label><br/>
...
class GrokAssistantController(http.Controller):
    ...
    def grok_config(self, **kw):
        api_key = escape(_get_param('grok_odoo_assistant.api_key', ''))
        model = escape(_get_param('grok_odoo_assistant.model', 'grok-4-latest'))
        temp = escape(_get_param('grok_odoo_assistant.temperature', '0.0'))
        allowed = escape(_get_param('grok_odoo_assistant.allowed_models', 'res.partner,sale.order,sale.order.line'))
        html = CONFIG_HTML.replace('%API_KEY%', api_key).replace('%MODEL%', model).replace('%TEMP%', temp).replace('%ALLOWED%', allowed)
