...
def _create_product(values):
    ...
    tmpl = env["product.template"].sudo().create({
        ...
        "categ_id": values.get("categ_id") or False,
                ...
                html = """<html><head><title>Grok Assistant</title></head><body style='font-family:system-ui;padding:16px'><h2>Grok Assistant Pro</h2><div id=\"chat\" style=\"border:1px solid #ddd;border-radius:10px;padding:1rem;min-height:320px;\"></div><script>document.getElementById(\"chat\").innerHTML=\"Template not found; but controller is healthy.\";</script></body></html>"""
...
class GrokController(http.Controller):
    ...
    def grok_ui(self, **kw):
        ...
        try:
                return request.render("grok_odoo_assistant_pr88.ui_page", {
    ...
    def grok_config(self, **kw):
        ...
        try:
                return request.render("grok_odoo_assistant_pr88.config_page", values)
            except Exception:
                html = """<html><body style='font-family:system-ui;padding:16px'><h2>Grok Assistant Settings</h2><p>Template not found; showing fallback.</p></body></html>"""
                return request.make_response(html, headers=[("Content-Type","text/html; charset=utf-8")])
