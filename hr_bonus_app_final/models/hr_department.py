...
class HrDepartment(models.Model):
    ...
    bonus_spent = fields.Float("Spent", compute="_compute_bonus_totals", store=False)
    bonus_remaining = fields.Float("Remaining", compute="_compute_bonus_totals", store=False)
    @api.depends("bonus_budget")
    def _compute_bonus_totals(self):
        Bonus = self.env["hr.bonus.request"].sudo()
        for dep in self:
            spent = 0.0
            data = Bonus.read_group([("department_id","=",dep.id), ("state","in",["approved","paid"])], ["amount:sum"], [])
            if data:
