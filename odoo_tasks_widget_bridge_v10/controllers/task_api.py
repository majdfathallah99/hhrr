...
class TaskWidgetAPI(http.Controller):
    ...
    # ---- New endpoints for saved items and refresh ----

    @http.route("/task_widget/refresh", type="http", auth="user", methods=["GET"], csrf=False, website=False)
    def refresh_saved(self, **kw):
        """Rebuild saved items for the current user then redirect to the Saved Items list."""
        user = request.env.user
        request.env["task.widget.item"].with_user(user.id).sudo().snapshot_for_user(user)
        action = request.env.ref("odoo_tasks_widget_bridge_v7.action_task_widget_saved_items").sudo()
